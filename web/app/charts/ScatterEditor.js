...
bluewave.charts.ScatterEditor = function(parent, config) {
  ...
  //** update
  //**************************************************************************
    this.update = function(scatterConfig){
        me.clear();

        if (!scatterConfig) scatterConfig = {};


      //Clone the config so we don't modify the original config object
        scatterConfig = JSON.parse(JSON.stringify(scatterConfig));


      //Update title
        setTitle(scatterConfig.chartTitle);


      //Update style
        if (!scatterConfig.style) scatterConfig.style = {};
        config.scatter.style = merge(scatterConfig.style, defaultConfig.scatter.style);
      ...
      //Update nodes
        for (var nodeID in scatterConfig.nodes) {
            if (scatterConfig.nodes.hasOwnProperty(nodeID)){
              ...
              //Add props to node
                var props = scatterConfig.nodes[nodeID];
      ...
      //Update paths and quantities
        ...
        for (var i=0; i<connections.length; i++){
            ...
            var link = scatterConfig.links[key];
    ...
    this.getConfig = function(){
      ...
      //Create basic config
        var scatterConfig = {
            ...
            links: {},
            chartTitle: getTitle(),
            style: config.scatter.style
        };


      //Update layout (html in the layout is not synced with the dom)
        for (var key in scatterConfig.layout) {
            if (scatterConfig.layout.hasOwnProperty(key)){
                var node = scatterConfig.layout[key];
  ...
  //** createScatter
  //**************************************************************************
    var createScatter = function(parent){
        ...
        scatterChart = new bluewave.charts.ScatterChart(dashboardItem.innerDiv, config.scatter);
    };
  ...
  //**************************************************************************
    var updateScatter = function(){
        ...
        scatterChart.update(config.scatter.style, data);
    };
  ...
  //**************************************************************************
    var editStyle = function(){
      ...
      //Create styleEditor as needed
        ...
        var scatterStyle = config.scatter.style;


        styleEditor.onChange = function(){};
        styleEditor.findField("linkColor").setValue(scatterStyle.links.color);
        styleEditor.findField("linkOpacity").setValue(scatterStyle.links.opacity*100);
        styleEditor.onChange = function(){
            var settings = styleEditor.getData();
            scatterStyle.links.color = settings.linkColor;
            scatterStyle.links.opacity = settings.linkOpacity/100;
            updateScatter();
        };
